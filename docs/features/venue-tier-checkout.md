...
- **Coupon Event Scope**: The coupon admin requires at least one event for `specific-events`; the API also verifies every selected event belongs to the coupon's club. The retired `new-users` option is hidden and rejected for newly configured coupons.
